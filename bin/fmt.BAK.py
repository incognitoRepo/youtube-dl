...
class FullCallUtil:
  ...
  def __init__(self):
    self.homepath = basepath.joinpath('src/youtube-dl')
    self.lst_o_init_filepaths = [
        downloader := self.homepath.joinpath('youtube_dl/downloader/__init__.py'),
        extractor := self.homepath.joinpath('youtube_dl/extractor/__init__.py'),
        postprocessor := self.homepath.joinpath('youtube_dl/postprocessor/__init__.py'),
        youtube_dl := self.homepath.joinpath('youtube_dl/__init__.py'),
    ]
  ...
  def clean_row(self,row):
    ...
    return juxt(
        lambda r: r.Index,
        lambda r: r.filepath,
        lambda r: int(r.line_number),
        lambda r: r.event_kind.strip(),
        lambda r: r.call_data[0].strip(),
        lambda r: r.og_index
    )(row)
...
def _fullcall_main():
  ...
  def entry():
    query,actions,outputs,filenames,write_func,pkldf = qcfg = QueryConfig().fullcall()
  ...
  return sns.entry
fullcall_main = _fullcall_main()

def fullcall_main_short():
  fcdf_agg = pd.read_pickle("fcdf_agg.pkl")
  lwr = pd.read_pickle("lwrdf.pkl")
  lwr2 = write_file(df=fcdf_agg,lst_w_regions=lwr)
  return lwr2
